...
def translate(line):
    ...
    if line[1]==':=':
        ...
        if line[2]=='CALL':
            if line[3]=='read' or line[3]=='print' or line[3]=='printD' or line[3]=='readD' or line[3]=='readC' or line[3]=='printC' or line[3]=='printS' or line[3]=='readS':
    ...
    if line[0]=='FUNCTION':
        return '%s:'%line[1]
    if line[0]=='CALL':
        if line[-1]=='read' or line[-1]=='print' or line[-1]=='readD' or line[-1]=='readC' or line[-1]=='printD' or line[-1]=='printC' or line[-1]=='printS' or line[-1]=='readS': 
            return '\taddi $sp,$sp,-4\n\tsw $ra,0($sp)\n\tjal %s\n\tlw $ra,0($sp)\n\taddi $sp,$sp,4'%(line[-1])
        else:
            return '\taddi $sp,$sp,-24\n\tsw $t0,0($sp)\n\tsw $ra,4($sp)\n\tsw $t1,8($sp)\n\tsw $t2,12($sp)\n\tsw $t3,16($sp)\n\tsw $t4,20($sp)\n\tjal %s\n\tlw $a0,0($sp)\n\tlw $ra,4($sp)\n\tlw $t1,8($sp)\n\tlw $t2,12($sp)\n\tlw $t3,16($sp)\n\tlw $t4,20($sp)\n\taddi $sp,$sp,24\n\tmove %s,$v0'%(line[-1],Get_R(line[0]))
    if line[0]=='ARG':
...
.data
str: .space 20
_prompt: .asciiz "Enter your input:"
_ret: .asciiz "\\n"
.globl main
.text
read:
